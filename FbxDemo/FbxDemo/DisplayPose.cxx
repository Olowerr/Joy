...
void DisplayPose(FbxScene* pScene)
{
    ...
    for (i = 0; i < lPoseCount; i++)
    {
        ...
        for (j = 0; j < lPose->GetCount(); j++)
        {
            ...
            if (lPose->IsBindPose())
            {
                const char* name = lPose->GetNodeName(j).GetCurrentName();
                Writer::AddBindPose(name, pScene->FindNodeByName(name));
            }
            for (k=0; k<4; k++)
            {
                ...
                if (lPose->IsBindPose())
                {
                    FbxMatrix bindPoseInv = lPose->GetMatrix(j).Inverse();
                    FbxVector4 rows = bindPoseInv.GetRow(k);
                    Writer::SetBindPose((float)rows[0], (float)rows[1], (float)rows[2], (float)rows[3], k, lPose->GetNodeName(j).GetCurrentName());
                }
            }
