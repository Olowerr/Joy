...
void DisplayGlobalTimeSettings(FbxGlobalSettings* pGlobalSettings)
{
    ...
    float rate = (float)FbxGetFrameRate(pGlobalSettings->GetTimeMode());
    int start = int(lStart.GetSecondDouble() * rate);
    int end =   int(lEnd.GetSecondDouble() * rate);

    Writer::SetAnimationTime(rate, start, end);
