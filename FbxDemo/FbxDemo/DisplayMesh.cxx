...
void DisplayPolygons(FbxMesh* pMesh)
{
	...
	const char* name = pMesh->GetNode()->GetName();
    ...
    for (i = 0; i < lPolygonCount; i++)
    {
		...
		for (j = 0; j < lPolygonSize; j++)
		{
			...
			DisplayInt("\n            Control Point Index: ", lControlPointIndex);
			DisplayInt("            Vertex Point Index: ", vertexId);

			Writer::AddVertex(pMesh->GetNode()->GetName(), lControlPointIndex);
			...
			else
			{
				...
				Writer::SetVertexPosition(name, lControlPoints[lControlPointIndex]);
			}
			...
			for (l = 0; l < pMesh->GetElementUVCount(); ++l)
			{
				...
				auto asd = leUV->GetMappingMode();
				auto qwe = leUV->GetReferenceMode();
				...
				case FbxGeometryElement::eByControlPoint:
					...
					case FbxGeometryElement::eDirect:
						...
						Writer::SetVertexUV(name, leUV->GetDirectArray().GetAt(lControlPointIndex));
						break;
					case FbxGeometryElement::eIndexToDirect:
						{
							int id = leUV->GetIndexArray().GetAt(lControlPointIndex);
							Display2DVector(header, leUV->GetDirectArray().GetAt(id));
							Writer::SetVertexUV(name, leUV->GetDirectArray().GetAt(id));
						...
						case FbxGeometryElement::eIndexToDirect:
							{
								...
								Writer::SetVertexUV(name, leUV->GetDirectArray().GetAt(lTextureUVIndex));
							}
							break;
						default:
							break; // other reference modes not shown here!
			...
			for( l = 0; l < pMesh->GetElementNormalCount(); ++l)
			{
				...
				if(leNormal->GetMappingMode() == FbxGeometryElement::eByPolygonVertex)
				{
					...
					case FbxGeometryElement::eDirect:
						...
						Writer::SetVertexNormal(name, leNormal->GetDirectArray().GetAt(vertexId));
						break;
					case FbxGeometryElement::eIndexToDirect:
						{
				...
				else if (leNormal->GetMappingMode() == FbxGeometryElement::eByControlPoint)
				{
					...
					case FbxGeometryElement::eDirect:
					{
						...
						Writer::SetVertexNormal(name, leNormal->GetDirectArray().GetAt(lControlPointIndex));
						break;
					}
					case FbxGeometryElement::eIndexToDirect:
					{
			...
			if (pMesh->GetElementTangentCount() && pMesh->GetElementBinormalCount())
				Writer::AddVertexBiNormals(name);
			...
			for( l = 0; l < pMesh->GetElementTangentCount(); ++l)
			{
				...
				if(leTangent->GetMappingMode() == FbxGeometryElement::eByPolygonVertex)
				{
					...
					case FbxGeometryElement::eDirect:
						...
						Writer::SetVertexTangent(name, leTangent->GetDirectArray().GetAt(vertexId));
						break;
					case FbxGeometryElement::eIndexToDirect:
						{
			...
			for( l = 0; l < pMesh->GetElementBinormalCount(); ++l)
			{
				...
				if(leBinormal->GetMappingMode() == FbxGeometryElement::eByPolygonVertex)
				{
					...
					case FbxGeometryElement::eDirect:
						...
						Writer::SetVertexBiTangent(name, leBinormal->GetDirectArray().GetAt(vertexId));
						break;
					case FbxGeometryElement::eIndexToDirect:
						{
    ...
    else
    {
        for (i = 0; i < lPolygonCount; i++)
        {
            ...
            for (l = 0; l < pMesh->GetElementMaterialCount(); l++)
            {
				...
				Writer::SetMatIdx(lMaterialElement->GetIndexArray().GetAt(i));
