...
void DisplayGenericInfo(FbxNode* pNode, int pDepth)
{
    ...
    lString += "\n";
    std::string nodeType = pNode->GetTypeName();
    if (nodeType == "Null")
    {
        std::string groupName = pNode->GetName();
        Writer::AddGroup(groupName);
    }
...
void DisplayProperties(FbxObject* pObject)
{
    ...
    std::string nameType = pObject->GetTypeName();
    ...
    while (lProperty.IsValid())
    {
        ...
        std::string attribute = lString.Buffer();
        ...
        case eFbxDouble3:
            {
                ...
                DisplayString("            Default Value: ", lBuf);
                if (nameType == "Light" && attribute == "Lcl Translation")
                {
                    Writer::SetLightPos(lDefault);
                }
                if (nameType == "Light" && attribute == "Lcl Rotation")
                {
                    Writer::SetLightDir(lDefault);
                }
                if (Writer::isGroupTrue() && attribute == "Lcl Translation")
                {
                    Writer::SetGroupTranslation(lDefault);
                }
                if (Writer::isGroupTrue() && attribute == "Lcl Rotation")
                {
                    Writer::SetGroupRotation(lDefault);
                }
                if (Writer::isGroupTrue() && attribute == "Lcl Scaling")
                {
                    Writer::SetGroupScale(lDefault);
                }
            }
            break;
