...
void DisplayAnimation(FbxAnimLayer* pAnimLayer, FbxNode* pNode, int parentIdx, bool isSwitcher = false);
...
{
    int i, c = pScene->GetSrcObjectCount<FbxAnimStack>();
    for (i = 0; i < c; i++)
    {
...
void DisplayAnimation(FbxAnimStack* pAnimStack, FbxNode* pNode, bool isSwitcher)
{
    ...
    for (l = 0; l < nbAnimLayers; l++)
    {
        ...
        DisplayAnimation(lAnimLayer, pNode, 0, isSwitcher);
    }
...
void DisplayAnimation(FbxAnimLayer* pAnimLayer, FbxNode* pNode, int parentIdx, bool isSwitcher)
{
    int lModelCount;
    FbxString lOutputString;
    ...
    int c = pNode->GetChildCount();
    for(lModelCount = 0; lModelCount < c; lModelCount++)
    {
        DisplayAnimation(pAnimLayer, pNode->GetChild(lModelCount), ++parentIdx, isSwitcher);
    }
}
...
void DisplayChannels(FbxNode* pNode, FbxAnimLayer* pAnimLayer, void (*DisplayCurve) (FbxAnimCurve* pCurve), void (*DisplayListCurve) (FbxAnimCurve* pCurve, FbxProperty* pProperty), bool isSwitcher)
{
    ...
    if (!isSwitcher)
    {
        ...
        if (lAnimCurve)
        {
            jointName = pNode->GetInitialName();
            checker = "TX";
            Writer::KeyframeTrue();
            //FBXSDK_printf("        TX\n");
            ...
            lNodeAttribute->GetAttributeType() == FbxNodeAttribute::ePatch)
        {
			...
			for(int lBlendShapeIndex = 0; lBlendShapeIndex<lBlendShapeDeformerCount; ++lBlendShapeIndex)
			{
				...
				for(int lChannelIndex = 0; lChannelIndex<lBlendShapeChannelCount; ++lChannelIndex)
				{
					...
					if(lAnimCurve)
					{
                        Writer::SetMorphFrames(lGeometry->GetNode()->GetName(), lAnimCurve, lChannelName);
    ...
    while (lProperty.IsValid())
    {
        if (lProperty.GetFlag(FbxPropertyFlags::eUserDefined))
        {
            ...
            Writer::AddProp(pNode->GetName(), lProperty);
...
void DisplayCurveKeys(FbxAnimCurve* pCurve)
{
    ...
    for(lCount = 0; lCount < lKeyCount; lCount++)
    {
        ...
        if (Writer::isKeyframeTrue())
        {
            Writer::SetKeyframeValues(pCurve->KeyGetValue(lCount), checker, int(lKeyTime.GetSecondDouble() * 30), lCount, lKeyCount, jointName);
        }
