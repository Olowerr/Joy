...
void DisplayCameraPositionAndOrientation(FbxCamera* pCamera, FbxNode* pTargetNode, FbxNode* pTargetUpNode)
{
    ...
    Writer::SetCameraPos(pCamera->Position.Get());
    ...
    else
    {
        ...
        auto p = pCamera->Position.Get();
        auto i = pCamera->InterestPosition.Get();
        FbxVector4 vec;
        vec.mData[0] = i.mData[0] - p.mData[0];
        vec.mData[1] = i.mData[1] - p.mData[1];
        vec.mData[2] = i.mData[2] - p.mData[2];
        vec.mData[3] = 0.0;
        vec.Normalize();
        Writer::SetFwVector(vec);
    }
    ...
    else
    {
        ...
        Writer::SetUpVector(pCamera->UpVector.Get());
    }
...
void DisplayViewingAreaControls(FbxCamera* pCamera)
{
    ...
    Writer::SetNearPlane((float)pCamera->NearPlane.Get());
    DisplayDouble("        Far Plane: ", pCamera->FarPlane.Get());
    Writer::SetFarPlane((float)pCamera->FarPlane.Get());
...
void DisplayApertureAndFilmControls(FbxCamera* pCamera)
{
    ...
    Writer::SetFOV((float)pCamera->FieldOfView.Get());
}
